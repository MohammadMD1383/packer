...
int main(int argc, char **argv) {
	...
	VERBOSE INFO_MSG("preparing output file");
	ofstream outputFile;
	path outputFilePath{vmap["output"].as<string>()};
	
	if (exists(outputFilePath)) {
		WARNING_MSG("output file '" << outputFilePath.string() << "' exists; overwrite it? [y/n]");
		loop {
			switch (readKey()) {
				case 'y':
					goto loop_end;
	...
	VERBOSE INFO_MSG("writing files count");
	...
	VERBOSE INFO_MSG("preparing input files");
	...
	transform(
		...
		[](const auto &item) {
			...
			if (!exists(p)) {
				ERROR_MSG("input file '" << p.string() << "' doesn't exist");
				exit(-2);
			}
			if (!is_regular_file(p)) {
				ERROR_MSG("'" << p.string() << "' is not a file");
				exit(-3);
			}
			return p;
		}
	...
	VERBOSE INFO_MSG("writing metadata");
	FileInfo fi{};
	...
	for (const auto &item: inputFilePaths) {
		VERBOSE INFO_MSG("packing file '" << item.string() << "'");
